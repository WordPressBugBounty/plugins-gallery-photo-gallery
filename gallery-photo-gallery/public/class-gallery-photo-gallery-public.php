...
class Gallery_Photo_Gallery_Public {
    ...
    public static function get_gallery_category_by_ids($ids){
        global $wpdb;
    ...
    protected function ays_get_gallery_content($gallery, $gallery_options, $gal_lightbox_options, $id){
        ...
        $gallery_options['enable_hover_opacity_mobile'] = ( isset( $gallery_options['enable_hover_opacity_mobile'] ) && $gallery_options['enable_hover_opacity_mobile'] == 'off') ? false : true;
        
        // Gallery hover opacity Mobile
        if ( $gallery_options['enable_hover_opacity_mobile'] ) {
            $hover_opacity_mobile = ( isset( $gallery_options['hover_opacity_mobile'] ) && $gallery_options['hover_opacity_mobile'] != '' ) ?  stripslashes( esc_attr( $gallery_options['hover_opacity_mobile'] ) ) : $hover_opacity;
        } else {
            $hover_opacity_mobile = $hover_opacity;
        }
        ...
        // Enable Gallery title Color Mobile
        $gallery_options['enable_ays_gallery_title_color_mobile'] = ( isset( $gallery_options['enable_ays_gallery_title_color_mobile'] ) && $gallery_options['enable_ays_gallery_title_color_mobile'] == 'off') ? false : true;
        
        // Gallery title Color Mobile
        if ( $gallery_options['enable_ays_gallery_title_color_mobile'] ) {
            $gallery_title_color_mobile = ( isset( $gallery_options['ays_gallery_title_color_mobile'] ) && $gallery_options['ays_gallery_title_color_mobile'] != '' ) ?  stripslashes( esc_attr( $gallery_options['ays_gallery_title_color_mobile'] ) ) : $gallery_title_color;
        } else {
        ...
        $gallery_options['enable_border_radius_mobile'] = ( isset( $gallery_options['enable_border_radius_mobile'] ) && $gallery_options['enable_border_radius_mobile'] == 'off') ? false : true;
        
        // Gallery Border radius Mobile
        if ( $gallery_options['enable_border_radius_mobile'] ) {
            $images_b_radius_mobile = ( isset( $gallery_options['border_radius_mobile'] ) && $gallery_options['border_radius_mobile'] != '' ) ?  stripslashes( esc_attr( $gallery_options['border_radius_mobile'] ) ) : $images_b_radius;
        } else {
            $images_b_radius_mobile = $images_b_radius;
        }
        ...
        $gallery_options['enable_thumbnail_title_size_mobile'] = ( isset( $gallery_options['enable_thumbnail_title_size_mobile'] ) && $gallery_options['enable_thumbnail_title_size_mobile'] == 'off') ? false : true;
        
        // Gallery Thumbnail title size Mobile
        if ( $gallery_options['enable_thumbnail_title_size_mobile'] ) {
            $thumbnail_title_size_mobile = ( isset( $gallery_options['thumbnail_title_size_mobile'] ) && $gallery_options['thumbnail_title_size_mobile'] != '' ) ?  stripslashes( esc_attr( $gallery_options['thumbnail_title_size_mobile'] ) ) : $thumbnail_title_size;
        } else {
            $thumbnail_title_size_mobile = $thumbnail_title_size;
        }
        ...
        $current_gallery_author_registered = "";
        ...
        $current_gallery_category_title = "";
        
        $gallery_cats = $gallery['category_ids'];
        $gallery_category_ids = isset( $gallery_cats ) && $gallery_cats != "" ? sanitize_text_field( $gallery_cats ) : '';

        if ( $gallery_category_ids != '' ) {
            $categories = self::get_gallery_category_by_ids( $gallery_category_ids );
            $category_names = wp_list_pluck( $categories, 'title' );

            $current_gallery_category_title = implode( ', ', $category_names );
        }
        ...
        if( !empty($gallery_current_author_data) ){
            ...
            if ( ! is_null( $current_gallery_user_data ) && $current_gallery_user_data ) {
                ...
                $current_gallery_author_registered = ( isset( $current_gallery_user_data->data->user_registered ) && $current_gallery_user_data->data->user_registered != '' ) ? sanitize_text_field( $current_gallery_user_data->data->user_registered ) : "";
            }
        }

        if ( ! empty( $current_gallery_author_website_url ) ) {
            $current_gallery_author_website_url = '<a href="'. $current_gallery_author_website_url .'" target="_blank">'. $current_gallery_author_website_url .'</a>';
        }
        ...
        $message_data = array(                    
            ...
            'current_gallery_author_registered'             => $current_gallery_author_registered,
            ...
            'current_gallery_category_title'                => $current_gallery_category_title,
        ...
        $gallery_options['enable_filter_thubnail_opt_mobile'] = ( isset( $gallery_options['enable_filter_thubnail_opt_mobile'] ) && $gallery_options['enable_filter_thubnail_opt_mobile'] == 'off' ) ? false : true;

        // Filter for thumbnail Mobile
        if ( $gallery_options['enable_filter_thubnail_opt_mobile'] ) {
            $filter_thubnail_opt_mobile = ( isset( $gallery_options['filter_thubnail_opt_mobile' ] ) && $gallery_options['filter_thubnail_opt_mobile' ] != '' ) ?  sanitize_text_field( $gallery_options['filter_thubnail_opt_mobile' ] ) : $filter_thubnail_opt;
        } else {
        ...
        if ( $gal_lightbox_options['enable_filter_lightbox_opt_mobile'] ) {
            $filter_lightbox_opt_mobile = ( isset( $gal_lightbox_options['filter_lightbox_opt_mobile' ] ) && $gal_lightbox_options['filter_lightbox_opt_mobile' ] != '' ) ?  sanitize_text_field( $gal_lightbox_options['filter_lightbox_opt_mobile' ] ) : $filter_lightbox_opt;
        } else {
            $filter_lightbox_opt_mobile = $filter_lightbox_opt;
        }
        ...
        switch ( $filter_thubnail_opt_mobile ) {
            case 'blur':
                $gpg_filter_image_mobile = 'blur(3px)';
                break;
            case 'brightness':
                $gpg_filter_image_mobile = 'brightness(200%)';
                break;
            case 'contrast':
                $gpg_filter_image_mobile = 'contrast(200%)';
                break;
            case 'grayscale':
                $gpg_filter_image_mobile = 'grayscale(100%)';
                break;
            case 'hue_rotate':
                $gpg_filter_image_mobile = 'hue-rotate(90deg)';
                break;
            case 'invert':
            ...
            case 'saturate':
                ...
                break;
            case 'sepia':
                $gpg_filter_image_mobile = 'sepia(100%)';
                break;                
            default:
                $gpg_filter_image_mobile = 'none';
                    break;
        }
        ...
        switch ( $filter_lightbox_opt_mobile ) {
            case 'blur':
                $gpg_filter_lightbox_image_mobile = 'blur(3px)';
                break;
            case 'brightness':
                $gpg_filter_lightbox_image_mobile = 'brightness(200%)';
                break;
            case 'contrast':
                $gpg_filter_lightbox_image_mobile = 'contrast(200%)';
                break;
            case 'grayscale':
                $gpg_filter_lightbox_image_mobile = 'grayscale(100%)';
                break;
            case 'hue_rotate':
                $gpg_filter_lightbox_image_mobile = 'hue-rotate(90deg)';
                break;
            case 'invert':
                $gpg_filter_lightbox_image_mobile = 'invert(100%)';
                break;
            case 'saturate':
                $gpg_filter_lightbox_image_mobile = 'saturate(8)';
                break;
            case 'sepia':
                $gpg_filter_lightbox_image_mobile = 'sepia(100%)';
                break;                
            default:
                $gpg_filter_lightbox_image_mobile = 'none';
                    break;
        }
        ...
        $gallery_view .= "
            ...
            <style>                
                ...
                @media screen and (max-width: 768px){
                    ...
                    .ays_image_title > span {
                        font-size:". $thumbnail_title_size_mobile ."px;
                    ...
                    .ays_mosaic_column_item_". $id ." img, 
                    .ays_masonry_item_". $id ." img{
                        filter: ". $gpg_filter_image_mobile .";
                    }
                    ...
                    div.ays_grid_row div.ays_grid_column_".$id." > img {
                        object-position: ".$gallery_img_position_mobile." !important;
                    }
                    ...
                    div.mosaic_".$id." div.ays_mosaic_column_item_".$id." div.ays_hover_mask,
                    div.ays_grid_row div.ays_grid_column_".$id." div.ays_hover_mask {
                        background: ". $this->hex2rgba( $hover_color_mobile, $hover_opacity_mobile ) .";
